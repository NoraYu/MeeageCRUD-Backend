...
public class PractisecrudApplication {
    ...
    @Bean
    ApplicationRunner init(MessageRepository messageRepository){
        return  args -> {
            Stream.of("How are you doing?", "You are the Best").forEach(contents -> {
                Message message = new Message();
                message.setContents(contents);
                messageRepository.save(message);
            });
            for(Message m:messageRepository.findAll()){
                System.out.println(m.getId()+m.getContents());
            }
